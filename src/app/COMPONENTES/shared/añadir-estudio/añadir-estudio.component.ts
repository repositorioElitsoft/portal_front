...
editarAcademica(id: number | undefined | null) {
  ...
  if (laboralToEdit) {
    ...
    const fechaInicio = new Date(laboralToEdit.inf_acad_fec_ini).toISOString().split('T')[0];
    const fechaFin = new Date(laboralToEdit.inf_acad_fec_fin).toISOString().split('T')[0];
